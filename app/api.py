...
async def predict(
    distance_fire_stations: float = Query(
        default=1500.0, description="Distance à la caserne la plus proche (m)"
    ),
    distance_rivers: float = Query(
        default=800.0, description="Distance à la rivière la plus proche (m)"
    ),
    distance_roads: float = Query(
        default=200.0, description="Distance à la route la plus proche (m)"
    ),
    distance_powerlines: float = Query(
        default=600.0, description="Distance à la ligne électrique la plus proche (m)"
    ),
    aspect: float = Query(
        default=180.0, description="Orientation de la pente (degrés)"
    ),
    elevation: float = Query(default=400.0, description="Altitude (m)"),
    pop_dens: float = Query(
        default=50.0, description="Densité de population (personnes/km²)"
    ),
    slope: float = Query(default=5.0, description="Pente (degrés)"),
    anom_max_temp: float = Query(
        default=1.5,
        description="Anomalie de température max (écart à la moyenne 30 ans)",
    ),
    anom_max_wind_vel: float = Query(
        default=0.3, description="Anomalie de vitesse du vent max"
    ),
    anom_avg_temp: float = Query(
        default=0.8, description="Anomalie de température moyenne"
    ),
    anom_avg_rel_hum: float = Query(
        default=-2.0, description="Anomalie d'humidité relative moyenne"
    ),
    anom_avg_soil: float = Query(
        default=-0.1, description="Anomalie d'humidité du sol moyenne"
    ),
    anom_sum_prec: float = Query(
        default=-5.0, description="Anomalie de précipitations cumulées"
    ),
    max_temp: float = Query(
        default=32.0, description="Température maximale du jour (°C)"
    ),
    max_wind_vel: float = Query(
        default=45.0, description="Vitesse maximale du vent (m/s)"
    ),
    avg_wind_angle: float = Query(
        default=180.0, description="Angle moyen du vent (degrés)"
    ),
    avg_rel_hum: float = Query(
        default=30.0, description="Humidité relative moyenne (%)"
    ),
    avg_soil: float = Query(
        default=0.12, description="Humidité du sol moyenne (m³/m³)"
    ),
    sum_prec: float = Query(
        default=0.0, description="Précipitations cumulées du jour (mm)"
    ),
    ...
    return (
        f"🔥 Ignition probable (probabilité : {proba:.1%})"
        if prediction == 1
        else f"✅ Pas d'ignition (probabilité : {proba:.1%})"
    )
